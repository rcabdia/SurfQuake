...
class RealManager:
    ...
    def compute_t_dist(self):
        dist_file = os.path.join(self.OUT_DIR, 't_dist.dat')
    ...
    def _move_real_output_to_default(self):
        ...
        for f in self._real_out_files_names:
            ...
            if os.path.isfile(src_f_path):
                out = shutil.move(src_f_path, os.path.join(self.OUT_DIR, f))
                out_file_locations.append(out)

    def _buffer_data_from_real(self):
        filename = os.path.join(self.OUT_DIR, 'phase_sel.txt')
    ...
    def save(self) -> str:
        filename = os.path.join(self.OUT_DIR, 'phase_sel_total.txt')
