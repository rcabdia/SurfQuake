...
from loc_flow_isp.sq_isola_tools.BayesISOLA.inverse_problem import invert
...
def run_inversion(self):
	...
	if self.threads > 1: # parallel
		...
		# results = [pool.apply_async(invert, args=(grid[i]['id'], d_shifts, norm_d, Cd_inv, Cd_inv_shifts, self.inp.nr,
		# 	self.d.components, self.inp.stations, self.d.npts_elemse, self.d.npts_slice, self.d.elemse_start_origin,
		# 	self.inp.event['t'], self.d.samprate, self.deviatoric, self.decompose, self.d.invert_displacement,
		# 										  grid[i]['path'])) for i in todo]

		results = [pool.apply_async(invert, args=(grid[i]['id'], d_shifts, norm_d, Cd_inv, Cd_inv_shifts, self.inp.nr,
			self.d.components, self.inp.stations, self.d.npts_elemse, self.d.npts_slice, self.d.elemse_start_origin,
			self.inp.event['t'], self.d.samprate, self.deviatoric, self.decompose, self.d.invert_displacement,
												  self.working_directory, self.from_axistra)) for i in todo]

		output = [p.get() for p in results]
	else: # serial
		output = []
		for i in todo:
			#res = invert(grid[i]['id'], d_shifts, norm_d, Cd_inv, Cd_inv_shifts, self.inp.nr, self.d.components,
			# self.inp.stations, self.d.npts_elemse, self.d.npts_slice, self.d.elemse_start_origin, self.inp.event['t'],
			# self.d.samprate, self.deviatoric, self.decompose, self.d.invert_displacement, grid[i]['path'])

			res = invert(grid[i]['id'], d_shifts, norm_d, Cd_inv, Cd_inv_shifts, self.inp.nr, self.d.components,
			 self.inp.stations, self.d.npts_elemse, self.d.npts_slice, self.d.elemse_start_origin, self.inp.event['t'],
			 self.d.samprate, self.deviatoric, self.decompose, self.d.invert_displacement, self.working_directory,
						 from_axistra=self.from_axistra)
			output.append(res)
